...
def load_model_state_ignore_mismatch(model, init_dict):
    ...
    def same_shape(a, b):
        return len(a.shape) == len(b.shape) and all(
            x == y for x, y in zip(a.shape, b.shape)
        )
    ...
    for k in init_dict:
        if k in name_to_param:
            if same_shape(init_dict[k], name_to_param[k]):
                ...
                logging.info(
                    "{} shape is not consistent, expected: {}; got "
    ...
    logging.info(
        "unique keys in init dict = {}; total = {}".format(
            pformat(unique_key_in_init_dict),
            len(unique_key_in_init_dict),
        )
    )
    result = model.load_state_dict(real_init_dict, strict=False)
    logging.info(
        "unique key (not initialized) in current model = {}".format(
...
def align_and_update_state_dicts(model_state_dict, loaded_state_dict):
    ...
    logging.info(
        "target model param = {}; name matched = {}; loaded = {}".format(
            len(model_state_dict), target_source_name_matched, len(loaded_state_dict)
        )
    )
    logging.info(
        "from loaded; ignore = {}".format(
