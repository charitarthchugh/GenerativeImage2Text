...
def create_projecton_layer(
    ...
    if visual_projection_type is None:
        visual_projection = nn.Linear(visual_feature_size, textual_feature_size)
...
class AutoRegressiveBeamSearch(object):
    ...
    def search(
        ...
        if not do_sample:
            ...
            start_predicted_classes = torch.multinomial(
                start_class_logits.softmax(dim=1), num_samples=self.beam_size
            )  # (batch_size, num_beams)
            start_top_logprobs = torch.gather(
                start_class_logprobs, -1, start_predicted_classes
            )  # (batch_size, num_beams)
        ...
        while predictions.shape[-1] < self.max_steps:
            ...
            if not do_sample:
                ...
                predicted_classes = torch.multinomial(
                    class_logits.softmax(dim=1), num_samples=self.per_node_beam_size
...
class TransformerDecoderTextualHead(TextualHead):
    ...
    def forward(
        ...
        projected_visual_features = (
            self.visual_projection(hidden_states) if hidden_states is not None else None
        )
        if return_dict:
        ...
        if not isinstance(
            self.transformer, torch.nn.modules.transformer.TransformerDecoder
        ):
        ...
        trans_out = self.transformer(
            ...
            memory_key_padding_mask=(
                hidden_valid_mask.logical_not()
                if hidden_valid_mask is not None
                else None
            ),
    ...
    def forward(self, feature, target):
        ...
        if debug_print:
            with torch.no_grad():
                ...
                logging.info(
                    "{}: iter={}, avg pos = {}, max loss = {}, min loss = {}".format(
                        self.log_prefix,
                        self.iter,
                        avg_prob,
                        self.max_loss,
...
class CaptioningModel(nn.Module):
    def __init__(
        ...
        if num_image_with_embedding:
            ...
            self.img_temperal_embedding = nn.ParameterList(
                nn.Parameter(torch.zeros(1, 1, self.textual.visual_feature_size))
                for _ in range(num_image_with_embedding)
            )
    ...
    def forward_one(self, batch, return_info=False):
        ...
        if "context" in batch:
            context_embedding = (
                ...
                if self.context_not_share_embedding
                else self.textual.embedding
            )
        ...
        if not self.training or (not self.scst):
            return self.forward_one_ce(
                batch, visual_features, visual_features_valid, return_info
            )
        else:
    ...
    def forward_one_scst(self, batch, visual_features, visual_features_valid):
        ...
        gt_res = list(
            zip(
                *[
                    [j_th_image_cap for j_th_image_cap in i_th_caption["caption"]]
    ...
    def forward_one_ce(
        self, batch, visual_features, visual_features_valid, return_info
    ):
        ...
        if self.training:
            ...
            if (
                self.verbose["num_has_image"] + self.verbose["num_no_image"]
            ) % 200 == 0:
                logging.info(self.verbose)
            hint = (
                "l"
                if "context_target_type" not in batch
                else batch["context_target_type"][0]
            )
            if has_image:
            ...
            if isinstance(logits, tuple) and len(logits) == 2:
                if self.prev_encoded_layers is None:
                    ...
                    self.prev_encoded_layers = [
                        torch.cat((p, c), dim=1)
                        for p, c in zip(self.prev_encoded_layers, logits[1])
                    ]
                # self.prev_encoded_layers = None
                logits = logits[0]
...
class GeneratorWithBeamSearch(object):
    ...
    def search(
        ...
        generated_hyps = [
            BeamHypotheses(
                num_keep_best, max_length, self.length_penalty, early_stopping=False
            )
        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_words = torch.multinomial(
                    F.softmax(scores, dim=-1), num_samples=per_node_beam_size
                )  # (batch_size * num_beams, TOPN_PER_BEAM)
                # Compute next scores
                ...
                beam_indices = (
                    torch.arange(num_beams, device=next_words.device) * vocab_size
                )
                ...
                for idx, score in zip(next_words[batch_ex], next_scores[batch_ex]):
                    ...
                    if word_id.item() in eos_token_ids or cur_len + 1 == max_length:
                        generated_hyps[batch_ex].add(
                            input_ids[batch_ex * num_beams + beam_id, :cur_len].clone(),
                            score.item(),
                        )
                    else:
        ...
        logprobs = (
            torch.zeros(batch_size, num_keep_best, dtype=torch.float)
            .fill_(-1e5)
            .to(input_ids.device)
        )
        all_best = []
        ...
        for i, hypotheses in enumerate(generated_hyps):
            ...
            _, best_indices = torch.topk(
                hyp_scores, min(num_keep_best, len(hyp_scores)), largest=True
            )
        ...
        decoded = input_ids.new(batch_size, num_keep_best, max_length).fill_(
            pad_token_id
        )
        for batch_idx, best in enumerate(all_best):
            for best_idx, hypo in enumerate(best):
                ...
                decoded[
                    batch_idx, best_idx, tgt_len[batch_idx, best_idx] - 1
                ] = eos_token_ids[0]
        if num_keep_best == 1:
...
class BeamHypotheses(object):
    ...
    def is_done(self, best_sum_logprobs):
        ...
        if len(self) < self.n_hyp:
            ...
            return self.worst_score >= best_sum_logprobs / self._length_norm(
                self.max_length
            )
