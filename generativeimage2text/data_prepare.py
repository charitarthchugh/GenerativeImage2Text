...
def get_imagenet_unique_nick_names():
    ...
    nick_names = [
        nick_name_overwrite[n]
        if n in nick_name_overwrite
        else get_nick_name(noffset_to_synset(n))
        for n in noffsets
    ]
...
def generate_imagenet_unique_names():
    ...
    write_to_file(
        "\n".join(nick_names), "./aux_data/imagenet/imagenet_unique_readable_names.txt"
    )
    ...
    def gen_rows():
        for i in tqdm(infos):
            payload = base64.b64encode(
                read_to_buffer(op.join(image_folder, i["filename"]))
            )
